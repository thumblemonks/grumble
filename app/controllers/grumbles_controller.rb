class GrumblesController < ApiController
  ...
  def create
    ...
    grumble.save!
    render_json({:grumble => grumble_attributes(grumble)}, :callback => 'grumbleCreated',
      :status => :created)
  end
...
private
  ...
  def grumble_attributes(grumble)
    grumble_data = {
      :id => grumble.uuid,
      :grumble_url => grumble_url(:id => grumble, :target_id => grumble.target),
      :grumbler_name => grumble.grumbler_name,
